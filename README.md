...
# 9.简单 Diff 算法
## 概念
- 通过 DOM 的移动来完成子节点的更新，比不断地执行子节点的卸载和挂载性能更好。 
- 移动节点：移动一个虚拟节点所对应的真实 DOM 节点，并不是移动虚拟节点本身。
- 为什么需要移动节点？-> 真实 DOM 仍然保持旧的一组子节点的顺序，新的一组子节点的顺序可能发生改变（针对同一 key 的节点），需要通过移动节点来完成真实 DOM 顺序的更新。
- 怎么判断一个节点是否需要移动？-> 在旧 children 中寻找具有相同 key 值节点的过程中，会遇到一个最大索引值。如果后续寻找过程中，存在索引值比当前遇到的最大索引值小的节点，则意味着该节点需要移动，见227页。

## 实现
- 对于两组子节点的更新的情况，怎么通过减少 DOM 操作提高性能？-> 遍历旧子节点数组，调用 patch 函数依次更新（适用于新旧子节点数量相同的情况）
- 新旧子节点数量不相同时，怎么改写 patchChildren？-> 遍历新、旧子节点数组长度较短的那一组（尽可能多地调用 patch 函数进行更新，而不是直接执行卸载和挂载函数）
- 怎么确定新旧两组子节点中节点的对应关系/如何移动节点（做到根据更新相同节点的内容，而不是根据索引更新内容，可能相同索引处根本就不是一个标签）？-> 引入 key 属性，找到可复用的节点
- 怎么找到需要移动的元素？-> 用 lastIndex 存储整个寻找过程中遇到的最大索引值。
- 如何移动节点？
  - 新旧节点都存在对真实 DOM 的引用 -> 通过旧子节点的 vnode.el 属性取得它对应的真实 DOM 节点。
  - 新 children 的顺序就是更新后真实 DOM 节点应有的顺序，移动细节见230页。
- 怎么添加新元素？-> 详细过程见235页
  - 找到新增节点 -> 增加 find 标识，代表是否在旧的一组子节点中找到可复用的节点。
  - 将新增节点挂载到正确的位置 -> 获取锚点元素。
- 怎么移除不存在的元素？-> 基本的更新结束时，遍历旧的一组子节点，然后去新的一组子节点中寻找具有相同 key 值的节点，若找不到，则删除该节点。

## 总结
- 简单 Diff 算法利用虚拟节点的 key 属性，尽可能地复用 DOM 元素，并通过移动 DOM 的方式来完成更新，从而减少不断地创新和销毁 DOM 元素带来的性能开销。但简单 Diff 算法仍有很多缺陷，这些缺陷可以通过双端 Diff 算法解决。

# 10.双端 Diff 算法
## 概念
- 双端 Diff 算法是一种同时对新旧两组子节点的两个端点进行比较的算法。
- 双端 Diff 相对于简单 Diff 的优势：减少了 DOM 移动的次数，图解见253到255页。
- 非理想状态是什么意思？-> while 循环时每轮比较都命中下面4个步骤（每轮中新、旧一组子节点中的头部或尾部的节点能够复用），只要命中不了就是非理想状态。
- 当前头部节点：旧的一组子节点中的头部节点所对应的真实 DOM 节点。

## 实现
- 双端 Diff 的前提需要什么？-> 需要四个索引值，分别指向新旧两组子节点的端点：newStartIdx、newEndIdx、oldStartIdx、oldEndIdx
- 理想状态下双端 Diff 算法的实现？-> patchKeyedChildren 函数中的 while 循环，满足循环条件时每一轮比较分为四个步骤（命中步骤n则执行该条件中的处理，图解见247到252页），每一轮结束后更新索引值。
  - 步骤1：比较旧的一组子节点中的第一个子节点与新的一组子节点中的第一个子节点
  - 步骤2：比较旧的一组子节点中的最后一个子节点与新的一组子节点中的最后一个子节点
  - 步骤3：比较旧的一组子节点中的第一个子节点与新的一组子节点中的最后一个子节点
  - 步骤4：比较旧的一组子节点中的最后一个子节点与新的一组子节点中的第一个子节点
- 非理想状态下怎么处理？-> 看非头部、非尾部的节点能否复用（遍历旧的一组子节点，试图寻找与 newStartNode 拥有相同 key 值的节点，找到就进入复用处理，找不到就是进入添加新元素的处理） -> while 循环中增加兜底的 else 处理，补充旧的一组子节点中头、尾部节点为 undefined 时的处理（移动 DOM 是基于旧的一组子节点对应的真实   DOM 的位置进行移动的，因此不需要新的一组子节点中头、尾部节点为 undefined 时的处理）。
- 怎么添加新元素？-> 将新元素挂载到当前头部节点之前，同时考虑目前双端 Diff 过程中由于不满足 while 循环而终止时遗漏新元素的情况（265到267页）-> 在 while 循环中增加 idxInOld 不存在时的逻辑，在 while 循环外增加“oldEndIdx < oldStartIdx && newStartIdx <= newEndIdx”的判断。
- 怎么移除不存在的元素？-> while 循环外增加“newEndIdx < newStartIdx && oldStartIdx <= oldEndIdx”的判断。

## 总结
- 双端 Diff 算法指的是在新旧两组子节点的四个端点之间分别进行比较，试图找到可复用的节点。

# 11.快速 Diff 算法
## 概念
- 快速 Diff 算法包含预处理过程：判断新旧两组子节点是否具有相同的前置节点和相同的后置节点（不需要进行 Diff 处理）。
- 理想情况：处理完相同的前置接节点或后置节点后，新旧两组子节点中总会有一组子节点全部被处理完。
- 复杂情况：非理想情况（相比于旧的，新的一组子节点中既有需要新增的，又有需要删除的）。
- Diff 算法（无论是哪种）的核心思想：
  - 判断是否有节点需要移动，以及如何移动
  - 找出需要被添加或删除的节点
- source 数组的作用：存储新的一组子节点中的节点在旧的一组子节点中的位置索引，后续将会使用它计算出一个最长递增子序列，并用于辅助完成 DOM 移动的操作。
- 已经更新过的节点数量/patched 应该小于新的一组子节点中需要更新的节点数量/count，一旦前者超过后者，说明有多余的节点，需要将它们卸载。
- 寻找 source 数组的最长递增子序列（例如 [0, 1]）的意义：在新的一组子节点中，重新编号后的索引值为 0 和 1 的这两个节点在更新前后顺序没有发生变化，即这两个索引处的节点不需要移动。

## 实现
- 怎么处理相同的前置节点？-> 一个索引 j，用一个 while 循环（从前往后遍历）找到相同的前置节点，调用 patch 函数进行更新。
- 怎么处理相同的后置节点？-> 两个索引 newEnd 和 oldEnd，用另一个 while 循环（从后往前遍历）找到相同的后置节点，调用 patch 函数进行更新。
- 理想情况怎么判断新的一组子节点中有新增节点？-> 276页
  - oldEnd < j：预处理过程中，所有的旧节点处理完毕。
  - newEnd >= j：预处理后，在新的一组子节点中，仍有未被处理的节点（新增节点）。
- 理想情况怎么判断新的一组子节点中有删除节点？-> 277页
  - newEnd < j：预处理过程中，所有的新节点处理完毕。
  - j <= oldEnd: 预处理后，在旧的一组子节点中，仍有未被处理的节点（删除节点）。
- 复杂情况下怎么判断是否需要进行 DOM 移动操作？
  - 计算新的一组节点中剩余未处理节点的数量 -> newEnd - j + 1。
  - source 数组怎么填充？-> 双层循环改进为索引表；理解 k - newStart 和 i 的含义。
  - 如何判断节点是否需要移动？-> 变量 moved、pos 和 patched 的含义，及 k 与 pos 的关系、patched 和 count 的关系。
- 复杂情况下如何移动元素？-> 利用 source 数组计算出一个最长递增子序列，用于 DOM 移动操作；创建索引 i 和 s。
